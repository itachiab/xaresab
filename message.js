  ...
  if (cmd) {
    ...
    if (!csd) {
      let dcs15 = new csdiscord.MessageEmbed()
        ...
        .setThumbnail()
        .setDescription(
          `**<a:_1:854448659236257802>   Botun Sahibi Benim Gibisinden Söylemde Bulunmak\n <a:_2:854448868464132166>  Botu Kötü Niyetli Kullanmayınız (Anlaşılırsa Karalisteye Alınırsınız) \n <a:_3:854449484730073129>  Tüm Mesajlarınız Kayıt Altına Alınır \n\n
      ...
      return message.channel.send(dcs15).then(sunucu => {
        ...
        csv.on("collect", async r => {
          message
            .reply(
              `**Kurallarımızı Kabul Ettiğiniz İçin Teşekkürler Botumuzu Artık Sorunsuz Bir Şekilde Kullana Bilirsiniz! Seninle Birlikte ${kural} Kişi Kabul Etmiş.** `
            )
    ...
    if (message.author.id !== ayarlar.sahip) {
      if (bakım) {
        return message.channel.send(
          `**Sizlere En İyi Hizmeti Verebilmek İçin Bakımdayız.\n❓ Bakım Sebebi: \`${bakım}\`\n Lütfen Daha Sonra Tekrar Deneyin.**`
        );
      }
    }
    ...
    if (karaliste == "aktif") {
      ...
      const webhook = new Discord.WebhookClient(
        "850832239637692437",
        "_xm-imQf4JT_87XJLwZ_pct6vbbvJGwrNVgjGcyfHx3xRYpjEttK8LegM7_dtOkm9ijL"
      );
    ...
    if (cmd.conf.enabled === false) {
      if (
        ...
        const embed = new Discord.MessageEmbed()
          .setDescription(
            `<a:cryptored:770187639801774080> **${cmd.help.name}** isimli komut şuanda geçici olarak kullanıma kapalıdır!`
          )
          .setColor("RED");
